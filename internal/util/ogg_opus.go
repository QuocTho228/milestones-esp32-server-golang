...
// NormalizeOpusSampleRate quy chuẩn hóa tần số lấy mẫu về tần số lấy mẫu tiêu chuẩn mà Opus hỗ trợ.
...
// PCM16ToOggOpus mã hóa dữ liệu PCM16 thành Ogg/Opus.
func PCM16ToOggOpus(samples []int16, sampleRate int, channels int, frameDurationMs int) ([]byte, error) {
	if channels < 1 || channels > 2 {
		return nil, fmt.Errorf("Opus chỉ hỗ trợ 1 hoặc 2 kênh, hiện tại: %d", channels)
	}
	...
	if frameSizePerChannel <= 0 {
		return nil, fmt.Errorf("thời lượng khung Opus không hợp lệ: %d ms", frameDurationMs)
	}
	...
	for offset := 0; offset < len(samples); offset += frameSize {
		...
		if err != nil {
			return nil, fmt.Errorf("mã hóa Opus thất bại: %v", err)
		}
		packet := make([]byte, n)
...
// WrapOggOpusPackets đóng gói các packet Opus thô thành luồng dữ liệu Ogg/Opus.
...
func buildOpusTagsPacket() []byte {
	vendor := []byte("milestones-mock-ai-server")
	var buf bytes.Buffer
