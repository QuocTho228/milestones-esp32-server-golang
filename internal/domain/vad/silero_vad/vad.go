...
func NewSileroVAD(config map[string]interface{}) (*SileroVAD, error) {
	...
	if modelPath == "" {
		return nil, errors.New("thiếu cấu hình đường dẫn model")
	}
...
func (s *SileroVAD) IsVADExt(pcmData []float32, sampleRate int, frameSize int) (bool, error) {
	...
	if s.closed || s.stream == nil || s.runtime == nil {
		return false, errors.New("thực thể Silero VAD chưa được khởi tạo")
	}
	if len(pcmData) == 0 {
		return false, nil
	}
...
func acquireRuntime(cfg speech.RuntimeConfig) (runtimeKey, *speech.Runtime, error) {
	...
	if shared, ok := runtimes[key]; ok {
		shared.refs++
		log.Debugf(
			"Silero VAD tái sử dụng Runtime dùng chung: model=%s, num_sessions=%d, refs=%d",
			key.modelPath,
			key.numSessions,
			shared.refs,
		)
	...
	log.Debugf(
		"Silero VAD tạo Runtime dùng chung: model=%s, num_sessions=%d, intra_threads=%d, inter_threads=%d, refs=1",
		key.modelPath,
		key.numSessions,
...
func resolveModelPath(modelPath string) (string, error) {
	...
	if modelPath == "" {
		return "", errors.New("thiếu cấu hình đường dẫn model")
	}
	...
	return "", fmt.Errorf("không tìm thấy file model: %s (đã thử: %s)", modelPath, strings.Join(candidates, ", "))
}
