...
func (s *ragflowSearcher) Search(
	...
	if len(errs) > 0 {
		log.Warnf("Truy vấn cơ sở tri thức RAGFlow thất bại một phần: %s", strings.Join(errs, "; "))
	}
	return ret, nil
}
...
func (s *ragflowSearcher) searchOneDataset(
	...
	if err != nil {
		return nil, fmt.Errorf("Tạo yêu cầu RAGFlow thất bại (dataset_id=%s): %w", datasetID, err)
	}
	...
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("RAGFlow trả về lỗi bất thường (dataset_id=%s): %d %s", datasetID, resp.StatusCode, string(bodyBytes))
	}
	...
	if err := json.Unmarshal(bodyBytes, &ragflowResp); err != nil {
		return nil, fmt.Errorf("Phân tích dữ liệu trả về từ RAGFlow thất bại (dataset_id=%s): %w", datasetID, err)
	}
	if ragflowResp.Code != 0 {
		return nil, fmt.Errorf("Yêu cầu RAGFlow thất bại (dataset_id=%s): code=%d message=%s", datasetID, ragflowResp.Code, strings.TrimSpace(ragflowResp.Message))
	}
