...
func (s *difySearcher) Search(
	...
	if len(errs) > 0 {
		log.Warnf("Truy vấn cơ sở tri thức Dify thất bại một phần: %s", strings.Join(errs, "; "))
	}
	return ret, nil
}
...
func (s *difySearcher) searchOneDataset(
	...
	if err != nil {
		return nil, fmt.Errorf("Tạo yêu cầu Dify thất bại (dataset_id=%s): %w", datasetID, err)
	}
	...
	if resp.StatusCode >= 400 {
		...
		resp.Body.Close()
		return nil, fmt.Errorf("Dify trả về lỗi bất thường (dataset_id=%s): %d %s", datasetID, resp.StatusCode, string(bodyBytes))
	}
	...
	resp.Body.Close()
	if decodeErr != nil {
		return nil, fmt.Errorf("Phân tích dữ liệu trả về từ Dify thất bại (dataset_id=%s): %w", datasetID, decodeErr)
	}
