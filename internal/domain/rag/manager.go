...
// Search nhóm các knowledge base theo provider, thực hiện truy vấn rồi tổng hợp và sắp xếp kết quả.
func Search(
	ctx context.Context,
	query string,
	topK int,
	...
	for provider, providerKBs := range grouped {
		...
		if searcher == nil {
			errs = append(errs, fmt.Sprintf("provider %s hiện chưa được hỗ trợ", provider))
			continue
		}
		providerConfig, ok := getProviderConfig(provider)
		if !ok {
			errs = append(errs, fmt.Sprintf("provider %s thiếu cấu hình toàn cục", provider))
			continue
		}

		providerHits, err := searcher.Search(totalCtx, q, topK, providerKBs, providerConfig)
		if err != nil {
			errs = append(errs, fmt.Sprintf("provider %s truy vấn thất bại: %v", provider, err))
			continue
		}
	...
	if len(errs) > 0 {
		log.Warnf("Một số provider truy vấn cơ sở tri thức thất bại: %s", strings.Join(errs, "; "))
	}
	return hits, nil
}
