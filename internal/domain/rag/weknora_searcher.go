...
func (s *weknoraSearcher) Search(
	...
	if len(errs) > 0 {
		log.Warnf("Truy vấn cơ sở tri thức Weknora thất bại một phần: %s", strings.Join(errs, "; "))
	}
	return ret, nil
}
...
func (s *weknoraSearcher) searchOneKnowledgeBase(
	...
	// Thang điểm (score) của WeKnora có thể thấp hơn nhiều so với ngưỡng 0~1
	// dùng chung cho các provider khác, nên không áp dụng lọc theo score ở đây.
	...
	if err != nil {
		return nil, fmt.Errorf("Tạo yêu cầu Weknora thất bại (knowledge_base_id=%s): %w", datasetID, err)
	}
	...
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("Weknora trả về lỗi bất thường (knowledge_base_id=%s): %d %s", datasetID, resp.StatusCode, string(bodyBytes))
	}
	...
	if err := json.Unmarshal(bodyBytes, &weknoraResp); err != nil {
		return nil, fmt.Errorf("Phân tích dữ liệu trả về từ Weknora thất bại (knowledge_base_id=%s): %w", datasetID, err)
	}
	if weknoraResp.Success != nil && !*weknoraResp.Success {
		...
		return nil, fmt.Errorf("Yêu cầu Weknora thất bại (knowledge_base_id=%s): %s", datasetID, msg)
	}
	if code, ok := parseCodeInt(weknoraResp.Code); ok && code != 0 {
		...
		return nil, fmt.Errorf("Yêu cầu Weknora thất bại (knowledge_base_id=%s): code=%d message=%s", datasetID, code, msg)
	}
