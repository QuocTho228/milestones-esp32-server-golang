...
// MilestonesProvider 小智TTS WebSocket Provider
// 支持流式文本转语音
...
// NewMilestonesProvider 创建新的小智TTS Provider
...
func (p *MilestonesProvider) handleTTSConnection(ctx context.Context, text string, outputChan chan []byte) error {
	...
	log.Debugf("发送milestones服务端消息: %v", listenMsg)
	...
	for {
		select {
		case <-ctx.Done():
			log.Debugf("milestones服务端消息ctx.Done(), 设备ID: %s", deviceId)
			return nil
		default:
		}
		...
		if msgType == websocket.TextMessage {
			log.Debugf("收到milestones服务端消息: %s", string(msg))
			var recvMsg RecvMsg
			...
			if recvMsg.Type == "tts" {
				if recvMsg.State == "stop" {
					log.Debugf("milestones服务端消息tts stop消息")
					return nil
				}
			}
			...
			if !firstFrameTs {
				firstFrameTs = true
				log.Debugf("tts耗时统计: milestones服务tts 第一个音频帧时间: %d", time.Now().UnixMilli()-startTs)
			}
			outputChan <- msg
			if i%20 == 0 {
				log.Debugf("milestones服务端音频消息, 已收到%d个音频帧", i)
			}
			i++
		}
	}
...
// SetVoice 设置音色参数（Milestones Provider 不支持动态设置音色）
func (p *MilestonesProvider) SetVoice(voiceConfig map[string]interface{}) error {
	return fmt.Errorf("Milestones TTS Provider 不支持动态设置音色")
}
