...
func (p *IndexTTSVLLMProvider) TextToSpeech(ctx context.Context, text string, sampleRate int, channels int, frameDuration int) ([][]byte, error) {
	...
	if len(frames) == 0 {
		return nil, fmt.Errorf("IndexTTS trả về âm thanh rỗng")
	}
	return frames, nil
}
...
func (p *IndexTTSVLLMProvider) TextToSpeechStream(ctx context.Context, text string, sampleRate int, channels int, frameDuration int) (outputChan chan []byte, err error) {
	if strings.TrimSpace(p.Voice) == "" {
		return nil, fmt.Errorf("indextts_vllm chưa cấu hình voice, vui lòng tạo giọng nói (voice) trước thông qua /audio/clone")
	}
	if frameDuration <= 0 {
	...
	go func() {
		...
		if reqErr != nil {
			log.Errorf("Yêu cầu IndexTTS thất bại: %v", reqErr)
			return
		}
		defer resp.Body.Close()
		if resp.StatusCode != http.StatusOK {
			msg, _ := io.ReadAll(io.LimitReader(resp.Body, 2048))
			log.Errorf("Yêu cầu IndexTTS thất bại: status=%d body=%s", resp.StatusCode, strings.TrimSpace(string(msg)))
			return
		}

		decoder, decErr := util.CreateAudioDecoderWithSampleRate(ctx, resp.Body, outputChan, frameDuration, "wav", sampleRate)
		if decErr != nil {
			log.Errorf("Lỗi khi tạo bộ giải mã âm thanh IndexTTS: %v", decErr)
			return
		}
		if runErr := decoder.Run(time.Now().UnixMilli()); runErr != nil {
			log.Errorf("Giải mã âm thanh IndexTTS thất bại: %v", runErr)
		}
	}()
...
func (p *IndexTTSVLLMProvider) SetVoice(voiceConfig map[string]interface{}) error {
	...
	return fmt.Errorf("cấu hình giọng nói không hợp lệ: thiếu voice/character")
}
