...
func (p *DoubaoTTSProvider) TextToSpeech(ctx context.Context, text string, sampleRate int, channels int, frameDuration int) ([][]byte, error) {
	...
	if len(frames) == 0 {
		return nil, fmt.Errorf("Doubao TTS trả về audio rỗng")
	}
	return frames, nil
}
...
func (p *DoubaoTTSProvider) streamHTTP(ctx context.Context, text string, sampleRate int, frameDuration int) (chan []byte, error) {
	...
	if voice == "" {
		return nil, fmt.Errorf("Doubao TTS thiếu voice")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("Chuyển đổi (serialize) request Doubao TTS thất bại: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, p.APIURL, bytes.NewReader(bodyBytes))
	if err != nil {
		return nil, fmt.Errorf("Tạo request Doubao TTS thất bại: %w", err)
	}
	...
	go func() {
		...
		if err != nil {
			log.Errorf("Request Doubao HTTP TTS thất bại: %v", err)
			close(outputChan)
			return
		}
		...
		if resp.StatusCode >= http.StatusBadRequest {
			...
			log.Errorf("Doubao HTTP TTS trả về lỗi: status=%d body=%s", resp.StatusCode, strings.TrimSpace(string(body)))
			close(outputChan)
			return
		}
...
func (p *DoubaoTTSProvider) decodeStreamResponse(ctx context.Context, body io.Reader, outputChan chan []byte, sampleRate int, frameDuration int) {
	...
	if err != nil {
		log.Errorf("Tạo bộ giải mã audio Doubao thất bại: %v", err)
		_ = pipeReader.Close()
		_ = pipeWriter.Close()
		close(outputChan)
		return
	...
	go func() {
		if err := decoder.Run(startTs); err != nil {
			log.Errorf("Giải mã audio Doubao thất bại: %v", err)
		}
	}()
...
func parseDoubaoTTSStreamEvent(raw []byte) (*doubaoTTSV3Event, error) {
	...
	if err := json.Unmarshal([]byte(line), &event); err != nil {
		return nil, fmt.Errorf("Phân tích (parse) sự kiện streaming Doubao TTS thất bại: %w", err)
	}
	if event.Code != 0 {
		msg := strings.TrimSpace(event.Message)
		if msg == "" {
			msg = fmt.Sprintf("Doubao TTS trả về mã lỗi %d", event.Code)
		}
...
func decodeDoubaoTTSAudioChunk(event *doubaoTTSV3Event) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Giải mã dữ liệu audio Doubao thất bại: %w", err)
	}
	return chunk, nil
}
