...
func (p *DifyLLMProvider) ResponseWithContext(ctx context.Context, sessionID string, dialogue []*schema.Message, _ []*schema.ToolInfo) chan *schema.Message {
	...
	go func() {
		...
		if strings.TrimSpace(query) == "" {
			sendLLMError(out, fmt.Errorf("dify query không được để trống"))
			return
		}
		...
		for event, eventErr := range sse.Read(resp.Body, nil) {
			if eventErr != nil {
				...
				sendLLMError(out, fmt.Errorf("đọc luồng (stream) dify thất bại: %w", eventErr))
				return
			}
			...
			if err := json.Unmarshal([]byte(data), &streamEvent); err != nil {
				log.Warnf("Phân tích sự kiện luồng (stream) dify thất bại: %v, data=%s", err, previewString(data, 256))
				continue
			}
...
func (p *DifyLLMProvider) stopTask(taskID, userID string) {
	...
	resp, err := p.httpClient.Do(req)
	if err != nil {
		log.Debugf("Request dừng task (stop task) của dify thất bại: %v", err)
		return
	}
	defer resp.Body.Close()
}
...
func (p *DifyLLMProvider) ResponseWithVllm(_ context.Context, _ []byte, _ string, _ string) (string, error) {
	return "", fmt.Errorf("dify provider không hỗ trợ khả năng vllm")
}
...
func buildDifyQuery(dialogue []*schema.Message) string {
	...
	// Ở chế độ hội thoại của Dify, chỉ gửi nội dung nhập của lượt hiện tại, không ghép lịch sử vào query.
	...
	// Dự phòng: nếu không tồn tại tin nhắn user, dùng tin nhắn cuối cùng có thể trích xuất được văn bản.
	for i := len(dialogue) - 1; i >= 0; i-- {
