...
// FunasrAdapter chuyển đổi (adapt) package funasr sang interface asr
type FunasrAdapter struct {
	engine *funasr.Funasr
}

// NewFunasrAdapter tạo một adapter FunASR mới
func NewFunasrAdapter(config map[string]interface{}) (AsrProvider, error) {
	// Tạo cấu hình FunasrConfig
...
// StreamingRecognize triển khai interface nhận dạng dạng streaming
func (a *FunasrAdapter) StreamingRecognize(ctx context.Context, audioStream <-chan []float32) (chan types.StreamingResult, error) {
	// Gọi phương thức StreamingRecognize của package funasr
