...
func Init(config *Config) error {
	...
	once.Do(func() {
		...
		log.Log().Info("Khởi tạo máy khách Redis thành công.")
	})
...
func GetClient() *redis.Client {
	...
	if globalClient == nil {
		log.Log().Warn("Máy khách Redis chưa được khởi tạo.")
		return nil
	}
...
func Close() error {
	...
	if globalClient != nil {
		...
		globalClient = nil
		if err != nil {
			log.Log().Errorf("Không thể đóng kết nối Redis: %v", err)
			return err
		}
		log.Log().Info("Kết nối Redis đã bị đóng.")
	}
...
func LogStats() {
	stats := Stats()
	if stats == nil {
		log.Log().Warn("Không thể lấy số liệu thống kê nhóm kết nối Redis.")
		return
	}

	log.Log().Infof("Thống kê nhóm kết nối Redis - Tổng số kết nối: %d, Kết nối rảnh rỗi: %d, Kết nối hết hạn: %d, Kết nối thành công: %d, Kết nối thất bại: %d, Kết nối hết thời gian chờ: %d",
