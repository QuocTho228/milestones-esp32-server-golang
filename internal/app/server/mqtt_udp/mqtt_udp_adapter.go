...
func (s *MqttUdpAdapter) connectAndRetry() {
	...
	for {
		...
		if token := client.Connect(); token.Wait() && token.Error() != nil {
			retryCount++
			Errorf("Kết nối máy chủ MQTT thất bại (%d thời gian): %v, Thử lại sau %d giây", retryCount, token.Error(), int(retryInterval.Seconds()))
			select {
			case <-s.stopCtx.Done():
				return
...
func (s *MqttUdpAdapter) handleLifecycleMessage(payload []byte) {
	...
	if err := json.Unmarshal(payload, &lifecycleEvent); err != nil {
		Errorf("Không thể phân tích cú pháp các thông báo vòng đời MQTT: %v", err)
		return
	}
	deviceID := strings.TrimSpace(lifecycleEvent.DeviceID)
	if deviceID == "" {
		Errorf("Thiếu tin nhắn vòng đời MQTT device_id: %s", string(payload))
		return
	}
	...
	case msgdata.MqttLifecycleStateOnline:
		...
		if err != nil {
			Errorf("Không thể xử lý sự kiện trực tuyến MQTT: device=%s err=%v", deviceID, err)
			return
		}
		if !acceptedOnline {
			return
	...
	default:
		Warnf("Bỏ qua các trạng thái vòng đời MQTT không xác định: device=%s state=%s", deviceID, lifecycleEvent.State)
	}
}
	...
	for {
		...
		case mqttMsg := <-s.msgChan:
			...
			if err := json.Unmarshal(mqttMsg.Payload(), &clientMsg); err != nil {
				Errorf("Lỗi phân tích cú pháp JSON: %v", err)
				continue
			}
			_, deviceId := s.getDeviceIdByTopic(mqttMsg.Topic())
			if deviceId == "" {
				Errorf("Lỗi khi phân tích mac_addr: %v", mqttMsg.Topic())
				continue
			}
			...
			if err != nil {
				Errorf("Đảm bảo transport MQTT trực tuyến thất bại: device=%s err=%v", deviceId, err)
				continue
			}
			if notifyOnline && s.onDeviceOnline != nil {
				s.onDeviceOnline(deviceId)
			...
			if existingSession != nil && clientMsg.Type == "hello" {
				...
				if err != nil {
					Errorf("Hello: tái tạo udpSession thất bại, deviceId: %s, err: %v", deviceId, err)
					continue
				}
				Debugf("Hello: Khởi tạo lại udpSession thành công., deviceId: %s, connID: %s", deviceId, newUdpSession.ConnId)
			}
