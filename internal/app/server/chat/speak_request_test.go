...
import (
	...
	data_audio "milestones-esp32-server-golang/internal/data/audio"
	data_client "milestones-esp32-server-golang/internal/data/client"
	msgdata "milestones-esp32-server-golang/internal/data/msg"
...
func TestHandleHelloRejectsUnsupportedTransportBeforeSession(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "loại transport không được hỗ trợ") {
...
func TestPrepareSpeakPathForInjectedSpeechForwardsAutoListenFlag(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() {
		errCh <- manager.prepareSpeakPathForInjectedSpeech("phát chủ động tự động nghe tiếp", true)
	}()
...
func TestPrepareSpeakPathForInjectedSpeechReusesPendingSpeakRequest(t *testing.T) {
	...
	go func() {
		errCh1 <- manager.prepareSpeakPathForInjectedSpeech("phát lần thứ nhất", false)
	}()

	_ = waitForServerMessage(t, conn, 0)

	go func() {
		errCh2 <- manager.prepareSpeakPathForInjectedSpeech("phát lần thứ hai", false)
	}()
...
func TestPrepareSpeakPathForInjectedSpeechAllocatesSessionIDWhenMissing(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() {
		errCh <- manager.prepareSpeakPathForInjectedSpeech("tự động gán phiên (session)", false)
	}()
...
func TestPrepareSpeakPathForInjectedSpeechTimesOut(t *testing.T) {
	...
	err := manager.prepareSpeakPathForInjectedSpeech("phát báo bị timeout", false)
	if err == nil {
		t.Fatal("expected prepareSpeakPathForInjectedSpeech to time out")
	}
	if !errors.Is(err, context.DeadlineExceeded) && err.Error() != "chờ speak_ready bị timeout" {
...
func TestInjectMessageWithSkipLlmFalseStillSendsSpeakRequest(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() {
		errCh <- manager.InjectMessage("cần đi qua LLM trước", false, false)
	}()
	...
	if item.text != "cần đi qua LLM trước" {
...
func TestInjectMessageWaitsForSessionBootstrapAfterSpeakReady(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() {
		errCh <- manager.InjectMessage("chờ phiên (session) được thiết lập", false, false)
	}()
	...
	if item.text != "chờ phiên (session) được thiết lập" {
		t.Fatalf("expected injected text to be queued, got %q", item.text)
	}
}
...
func TestInjectMessageTimesOutWhenSessionBootstrapDoesNotFinish(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() {
		errCh <- manager.InjectMessage("phiên (session) bị timeout", false, false)
	}()
	...
	case err := <-errCh:
		...
		if !strings.Contains(err.Error(), "chờ ChatSession thiết lập bị timeout") {
