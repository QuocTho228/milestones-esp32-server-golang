...
func TestHandleToolResultAcceptsPlainText(t *testing.T) {
	...
	result, ok := manager.handleToolResult("văn bản thông thường trả về")
	if !ok {
	...
	if textContent.Text != "văn bản thông thường trả về" {
...
func TestHandleToolResultAcceptsMCPJSON(t *testing.T) {
	...
	result, ok := manager.handleToolResult(`{"content":[{"type":"text","text":"phản hồi json"}],"isError":false}`)
	if !ok {
	...
	if textContent.Text != "phản hồi json" {
...
func TestGetMessagesUsesToolRoundMessagesInNoneMode(t *testing.T) {
	...
	user := schema.UserMessage("Giúp tôi kiểm tra thời tiết Thượng Hải")
	...
	toolMsg := schema.ToolMessage("Hôm nay Thượng Hải nhiều mây, 22 độ", "call_weather_1")
...
func TestAppendToolRoundMessagesAccumulatesInOrder(t *testing.T) {
	user := schema.UserMessage("Giúp tôi đặt báo thức")
	assistant := schema.AssistantMessage("", []schema.ToolCall{
		{
			ID:   "call_alarm_1",
			Type: "function",
	...
	toolMsg := schema.ToolMessage("Báo thức đã được đặt vào 7 rưỡi sáng", "call_alarm_1")
...
func newTestLLMManager(memoryMode string) *LLMManager {
	return &LLMManager{
		clientState: &data_client.ClientState{
			...
			SystemPrompt: "Bạn là một trợ lý thử nghiệm",
		},
	}
}