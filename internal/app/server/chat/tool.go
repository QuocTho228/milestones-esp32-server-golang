...
// handleToolCallResponse Xử lý phản hồi của lệnh gọi công cụ (tool call)
func (l *LLMManager) handleToolCallResponse(ctx context.Context, respMsg *schema.Message, tools []schema.ToolCall, executor *toolCallExecutor) (toolCallResponseSummary, error) {
	...
	log.Infof("Xử lý %d lệnh gọi công cụ (tool call)", len(tools))
	if executor == nil {
	...
	// Chỉ thêm vào messageList khi respMsg có nội dung (Content không rỗng hoặc ToolCalls không rỗng)
	// Tránh lưu message assistant rỗng gây ra lỗi 400 ở lần gọi LLM tiếp theo
	...
	if len(messageList) > 0 {
		for _, msg := range messageList {
			// Lọc bỏ message assistant có Content rỗng, tránh lưu vào lịch sử
			// Message assistant rỗng sẽ gây ra lỗi 400 khi gọi LLM tiếp theo
			if msg != nil && msg.Role == schema.Assistant && msg.Content == "" && len(msg.ToolCalls) == 0 {
				log.Debugf("Bỏ qua việc lưu message assistant rỗng")
				continue
			}
	...
	// Nếu lệnh gọi công cụ thành công và không bị đánh dấu dừng xử lý, thì tiếp tục gọi LLM
...
func (e *toolCallExecutor) executeToolCall(order int, toolCall schema.ToolCall) toolCallExecutionResult {
	...
	if !ok || toolObj == nil {
		log.Errorf("Không tìm thấy công cụ (tool): %s", toolName)
		resultMessage.Content = fmt.Sprintf("Không tìm thấy công cụ (tool): %s", toolName)
		return toolCallExecutionResult{order: order, message: resultMessage}
	}

	log.Infof("Thực hiện yêu cầu gọi công cụ (tool call): %s, tham số: %+v", toolName, toolCall.Function.Arguments)
	startTs := time.Now().UnixMilli()
	fcResult, err := toolObj.InvokableRun(e.toolCtx, toolCall.Function.Arguments)
	if err != nil {
		log.Errorf("Gọi công cụ (tool call) thất bại: %v", err)
		resultMessage.Content = fmt.Sprintf("Công cụ %s gọi thất bại: %v", toolName, err)
		return toolCallExecutionResult{order: order, message: resultMessage}
	}
	costTs := time.Now().UnixMilli() - startTs
	if len(fcResult) > 2048 {
		log.Infof("Kết quả gọi công cụ, độ dài: %d, thời gian xử lý: %dms", len(fcResult), costTs)
	} else {
		log.Infof("Kết quả gọi công cụ %s, thời gian xử lý: %dms", fcResult, costTs)
	}
	...
	if mcpResp, ok := e.manager.handleLocalToolResult(fcResult); ok {
		...
		if toolCallResult.IsError {
			log.Errorf("Gọi công cụ (tool call) thất bại: %s, cờ báo lỗi: %t", fcResult, toolCallResult.IsError)
		}
	...
	if len(contentList) > 0 {
		...
		for _, content := range contentList {
			if audioContent, ok := content.(mcp_go.AudioContent); ok {
				log.Debugf("Công cụ %s trả về tài nguyên audio, độ dài: %d", toolName, len(audioContent.Data))
				mcpContent = "Thực thi thành công"
				if err := e.manager.handleAudioContent(e.ctx, toolName, audioContent, &e.mediaWg); err != nil {
					log.Errorf("mcp phát tài nguyên audio thất bại: %v", err)
			...
			if resourceLink, ok := content.(mcp_go.ResourceLink); ok {
				log.Debugf("Công cụ %s trả về resource link: %+v", toolName, resourceLink)
				mcpContent = "Thực thi thành công"
			...
			if textContent, ok := content.(mcp_go.TextContent); ok {
				log.Debugf("Công cụ %s trả về tài nguyên văn bản, độ dài: %s", toolName, textContent.Text)
...
func (l *LLMManager) handleResourceLink(ctx context.Context, resourceLink mcp_go.ResourceLink, toolCall tool.InvokableTool, wg *sync.WaitGroup) error {
	...
	if l.session == nil || l.session.mediaPlayer == nil {
		wg.Done()
		return fmt.Errorf("session media player chưa được khởi tạo")
	}
...
func (l *LLMManager) handleAudioContent(ctx context.Context, realMusicName string, audioContent mcp_go.AudioContent, wg *sync.WaitGroup) error {
	...
	if err != nil {
		wg.Done()
		log.Errorf("Giải mã dữ liệu audio thất bại: %v", err)
		return err
	}
...
func buildMediaSourceFromAudioContent(title string, audioContent mcp_go.AudioContent) (MediaSourceDescriptor, error) {
	...
	if err != nil {
		return MediaSourceDescriptor{}, fmt.Errorf("Giải mã dữ liệu audio thất bại: %v", err)
	}
...
func buildMediaSourceFromResourceLink(resourceLink mcp_go.ResourceLink, toolCall tool.InvokableTool) (MediaSourceDescriptor, error) {
	...
	if !ok || mcpTool == nil {
		return MediaSourceDescriptor{}, fmt.Errorf("Phát resource link chỉ hỗ trợ công cụ MCP từ xa")
	}
...
func (l *LLMManager) handleLocalToolResult(toolResult string) (MCPResponse, bool) {
	// Nếu là định dạng phản hồi mcp, thì phân tích (parse)
