...
CGO_LDFLAGS="-lm" go run -buildvcs=false -tags nolibopusfile ./test/auto_test \
  -runner auto \
  -server ws://127.0.0.1:8990/milestones/v1/ \
...
| Case                                         | 输入                                                            | 期望输出 / 断言                                                                            |
| -------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `ota_metadata`                               | POST `/milestones/ota/`，携带设备信息、`Device-Id`、`Client-Id` | 返回 `websocket.url`、`server_time`、`firmware.version`；启用时校验 `activation` 和 `mqtt` |
| `ota_activate_invalid_algorithm`             | POST `/milestones/ota/activate`，提交非法 `algorithm`           | HTTP 400                                                                                   |
...
- 覆盖场景：调用 `/milestones/ota/activate`，提交非法 `algorithm`。
- 核心校验：
  - 服务端返回 `400`
  - 激活接口的参数校验链路生效
...
CGO_LDFLAGS="-lm" go run -buildvcs=false -tags nolibopusfile ./test/auto_test \
  -runner auto \
  -server ws://localhost:9990/milestones/v1/ \
  -device smoke-agent \
  -cases agent_ws_endpoint_mcp \
  -endpoint_auth_token milestones_mcp_openclaw_secret_key
```
