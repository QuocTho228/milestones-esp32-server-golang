...
用于对接 `test/milestones_openclaw/milestones-integration/openclaw-channel` 插件的 WebSocket 测试服务。
...
```bash
go run ./test/test_openclaw_server -addr :18080 -jwt-secret milestones_admin_secret_key
```

输出详细 WebSocket 调试日志：

```bash
go run ./test/test_openclaw_server -addr :18080 -jwt-secret milestones_admin_secret_key -verbose
```

## 生成 token（测试）

```bash
node test/milestones_openclaw/milestones-integration/generate-token.js 1 main agent_main
```

或显式指定与服务端一致的密钥（推荐）：

```bash
JWT_SECRET=milestones_admin_secret_key \
node test/milestones_openclaw/milestones-integration/generate-token.js 1 main agent_main
```

把输出的 `Token` 配到插件配置里：

- `channels.milestones.url = ws://127.0.0.1:18080/ws/openclaw`
- `channels.milestones.token = <token>`
