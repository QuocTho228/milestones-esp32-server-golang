...
func loadSecretKey() ([]byte, error) {
	...
	if raw == "" {
		return nil, fmt.Errorf("Biến môi trường %s chưa được đặt", marketSecretEnv)
	}
	...
	rawKey := []byte(raw)
	if len(rawKey) != 32 {
		return nil, fmt.Errorf("Giá trị của %s phải là 32 byte gốc hoặc Base64 (32 byte)", marketSecretEnv)
	}
...
func EncryptText(plain string) (ciphertextB64, nonceB64 string, err error) {
	...
	if _, err := io.ReadFull(rand.Reader, nonce); err != nil {
		return "", "", fmt.Errorf("Tạo random nonce thất bại: %w", err)
	}
...
func DecryptText(ciphertextB64, nonceB64 string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("Giải mã bản mã không thành công: %w", err)
	}
