...
func FetchJSON(ctx context.Context, endpoint string, headers map[string]string, opts HTTPOptions) (interface{}, error) {
	...
	if opts.JSONBody != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("Nội dung yêu cầu tuần tự hóa không thành công: %w", err)
		}
	...
	if len(opts.Query) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("Không thể phân tích cú pháp URL yêu cầu: %w", err)
		}
		q := u.Query()
	...
	if err != nil {
		return nil, fmt.Errorf("Không đọc được phản hồi: %w", err)
	}
	...
	if err := json.Unmarshal(body, &data); err != nil {
		return nil, fmt.Errorf("Phân tích cú pháp JSON không thành công: %w", err)
	}
	return data, nil
}
