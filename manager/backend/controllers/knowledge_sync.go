...
func parseKnowledgeProviderConfigPayload(cfg *models.Config) (*models.Config, map[string]interface{}, error) {
	if cfg == nil {
		return nil, nil, fmt.Errorf("Cấu hình provider cơ sở tri thức trống")
	}
...
func resolveKnowledgeProviderForKB(db *gorm.DB, kb *models.KnowledgeBase) (string, *models.Config, map[string]interface{}, error) {
	if kb == nil {
		return "", nil, nil, fmt.Errorf("Dữ liệu cơ sở tri thức trống")
	}
	...
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			if provider == "" {
				return "", nil, nil, fmt.Errorf("Không tìm thấy cấu hình provider cơ sở tri thức đã được bật")
			}
			return "", nil, nil, fmt.Errorf("Không tìm thấy cấu hình provider cơ sở tri thức đã được bật: %s", provider)
		}
		return "", nil, nil, fmt.Errorf("Lấy cấu hình provider cơ sở tri thức thất bại: %w", err)
	}
...
func syncKnowledgeBaseDeleteBestEffort(db *gorm.DB, kb *models.KnowledgeBase) error {
	...
	default:
		return fmt.Errorf("Đồng bộ xóa cơ sở tri thức hiện chưa hỗ trợ provider: %s", provider)
	}
}
...
func syncKnowledgeBaseWithProvider(db *gorm.DB, kb *models.KnowledgeBase) (*knowledgeProviderSyncResult, error) {
	...
	default:
		return nil, fmt.Errorf("Đồng bộ cơ sở tri thức hiện chưa hỗ trợ provider: %s", provider)
	}
}

func persistKnowledgeSyncState(db *gorm.DB, kb *models.KnowledgeBase, result *knowledgeProviderSyncResult, syncErr error) error {
	if kb == nil || kb.ID == 0 {
		return fmt.Errorf("Thực thể cơ sở tri thức không hợp lệ")
	}
...
func syncKnowledgeBaseToDify(cfg *difyKnowledgeSyncConfig, kb *models.KnowledgeBase) (*knowledgeProviderSyncResult, error) {
	if kb == nil {
		return nil, fmt.Errorf("Dữ liệu cơ sở tri thức trống")
	}
	...
	// Cho phép đồng bộ cơ sở tri thức rỗng: chỉ đảm bảo dataset tồn tại, không tạo/cập nhật tài liệu.
	if content == "" {
		now := time.Now()
...
func deleteKnowledgeBaseFromDify(cfg *difyKnowledgeSyncConfig, kb *models.KnowledgeBase) error {
	if kb == nil {
		return fmt.Errorf("Dữ liệu cơ sở tri thức trống")
	}
	datasetID := strings.TrimSpace(kb.ExternalKBID)
	documentID := strings.TrimSpace(kb.ExternalDocID)
	if datasetID == "" {
...
func syncKnowledgeBaseToWeknora(cfg *weknoraKnowledgeSyncConfig, kb *models.KnowledgeBase) (*knowledgeProviderSyncResult, error) {
	...
	// Cho phép đồng bộ cơ sở tri thức rỗng: chỉ đảm bảo cơ sở tri thức tồn tại, không tạo tài liệu.
	if content == "" {
		now := time.Now()
		result.LastSyncedAt = &now
		return result, nil
...
func syncKnowledgeDocumentBestEffort(db *gorm.DB, kbID, docID uint) error {
	...
	if err := db.Where("id = ? AND knowledge_base_id = ?", docID, kbID).First(&doc).Error; err != nil {
		...
		return fmt.Errorf("Tải tài liệu cơ sở tri thức thất bại: %w", err)
	}
	...
	case "weknora":
		...
		if err := db.Model(&models.KnowledgeBase{}).Where("id = ?", kb.ID).Updates(map[string]interface{}{
			...
			return fmt.Errorf("Cập nhật external_doc_id của cơ sở tri thức thất bại: %w", err)
		}
		return nil

	default:
		err := fmt.Errorf("Đồng bộ tài liệu cơ sở tri thức hiện chưa hỗ trợ provider: %s", provider)
		return failUpload(strings.TrimSpace(doc.ExternalDocID), err)
	}
}
...
func syncKnowledgeDocumentDeleteBestEffort(db *gorm.DB, kb models.KnowledgeBase, doc models.KnowledgeBaseDocument) error {
	...
	case "weknora":
		...
		return nil
	default:
		return fmt.Errorf("Đồng bộ xóa tài liệu cơ sở tri thức hiện chưa hỗ trợ provider: %s", provider)
	}
}
...
func ensureDifyDatasetForKnowledgeBase(db *gorm.DB, kb *models.KnowledgeBase, client *http.Client, cfg *difyKnowledgeSyncConfig) (string, error) {
	if kb == nil {
		return "", fmt.Errorf("Cơ sở tri thức trống")
	}
	datasetID := strings.TrimSpace(kb.ExternalKBID)
	if datasetID != "" {
	...
	if err := db.Model(&models.KnowledgeBase{}).Where("id = ?", kb.ID).Updates(updates).Error; err != nil {
		return "", fmt.Errorf("Cập nhật dataset_id của cơ sở tri thức thất bại: %w", err)
	}
...
func createDifyDocumentByFile(client *http.Client, cfg *difyKnowledgeSyncConfig, datasetID, fileName string, fileData []byte) (string, error) {
	...
	meta := map[string]interface{}{
		"name": fileName,
		// Dify create-by-file yêu cầu bắt buộc phải kèm theo process_rule.
...
func isDifyDatasetEmpty(client *http.Client, cfg *difyKnowledgeSyncConfig, datasetID string) (bool, error) {
	...
	// Một số bản triển khai có thể trả về cấu trúc khác nhau, thực hiện phân tích dự phòng một lần
...
func uploadRagflowDocumentByBytes(client *http.Client, cfg *ragflowKnowledgeSyncConfig, datasetID, fileName string, fileData []byte) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("Tải tài liệu RAGFlow lên thất bại(dataset_id=%s): %w", datasetID, err)
	}
	...
	if documentID == "" {
		return "", fmt.Errorf("Tải tài liệu RAGFlow lên thất bại: phản hồi thiếu document_id, body=%s", string(body))
	}
	return documentID, nil
}
...
func createWeknoraKnowledgeBase(client *http.Client, cfg *weknoraKnowledgeSyncConfig, kb *models.KnowledgeBase) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("Tạo cơ sở tri thức Weknora thất bại: %w", err)
	}
	...
	if kbID == "" {
		return "", fmt.Errorf("Tạo cơ sở tri thức Weknora thất bại: phản hồi thiếu id, body=%s", string(body))
	}
	return kbID, nil
}
...
func updateWeknoraKnowledgeBase(client *http.Client, cfg *weknoraKnowledgeSyncConfig, kbID string, kb *models.KnowledgeBase) error {
	...
	if kbID == "" {
		return fmt.Errorf("ID cơ sở tri thức weknora trống")
	}
	...
	// Tương thích API phiên bản cũ: nếu định dạng mới (update payload kèm config) thất bại, thử lại một lần theo định dạng phẳng cũ.
	if shouldRetryWeknoraLegacyUpdate(status, body, err) {
		...
		return fmt.Errorf(
			"Cập nhật cơ sở tri thức Weknora thất bại(knowledge_base_id=%s): primary_err=%v; fallback_status=%d fallback_err=%v fallback_body=%s",
			kbID,
			err,
			legacyStatus,
			legacyErr,
	...
	return fmt.Errorf("Cập nhật cơ sở tri thức Weknora thất bại(knowledge_base_id=%s): %w", kbID, err)
}
...
func deleteWeknoraKnowledgeBase(client *http.Client, cfg *weknoraKnowledgeSyncConfig, kbID string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Xóa cơ sở tri thức Weknora thất bại(knowledge_base_id=%s): %w", kbID, err)
	}
	return nil
}
...
func waitWeknoraKnowledgeParsed(client *http.Client, cfg *weknoraKnowledgeSyncConfig, knowledgeID string) error {
	knowledgeID = strings.TrimSpace(knowledgeID)
	if knowledgeID == "" {
		return fmt.Errorf("ID tài liệu weknora trống")
	}
	...
	for {
		...
		case "pending", "processing", "":
			if time.Now().After(deadline) {
				return fmt.Errorf("Chờ phân tích tài liệu Weknora quá thời gian(knowledge_id=%s timeout_ms=%d)", knowledgeID, timeout.Milliseconds())
			}
			time.Sleep(interval)
		default:
			if time.Now().After(deadline) {
				return fmt.Errorf("Chờ phân tích tài liệu Weknora quá thời gian(knowledge_id=%s status=%s timeout_ms=%d)", knowledgeID, status, timeout.Milliseconds())
			}
			time.Sleep(interval)
		}
	}
...
func buildWeknoraKnowledgeBaseConfigPayload(cfg *weknoraKnowledgeSyncConfig) map[string]interface{} {
	config := map[string]interface{}{
		...
		// Trong ví dụ tài liệu WeKnora PUT, trường này nằm trong config; khi không có model thì truyền chuỗi rỗng.
...
func doWeknoraJSONRequest(client *http.Client, method, endpoint, apiKey string, payload interface{}, out interface{}) (int, []byte, error) {
	...
	if payload != nil {
		...
		if err != nil {
			return 0, nil, fmt.Errorf("Mã hóa nội dung yêu cầu (request body) thất bại: %w", err)
		}
	...
	if err != nil {
		return 0, nil, fmt.Errorf("Tạo yêu cầu (request) thất bại: %w", err)
	}
...
func doWeknoraMultipartFileRequest(
	...
	if err := writer.Close(); err != nil {
		return 0, nil, fmt.Errorf("Đóng form writer thất bại: %w", err)
	}
