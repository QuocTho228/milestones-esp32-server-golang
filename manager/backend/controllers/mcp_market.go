...
import (
	...
	mcpmarket "milestones/manager/backend/services/mcp_market"
...
const (
	defaultMCPConfigName = "Cấu hình toàn cục MCP"
...
func (ac *AdminController) GetMCPMarkets(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Lấy danh sách kết nối chợ MCP thất bại"})
		return
	}
...
func (ac *AdminController) CreateMCPMarket(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Tuần tự hóa cấu hình chợ thất bại"})
		return
	}
	...
	if err := ac.DB.Create(&config).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Tạo kết nối chợ MCP thất bại"})
		return
	}
...
func (ac *AdminController) UpdateMCPMarket(c *gin.Context) {
	...
	if err := ac.DB.Where("id = ? AND type = ?", id, mcpmarket.MCPMarketConfigType).First(&existing).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Kết nối chợ MCP không tồn tại"})
		return
	}

	existingCfg, err := parseStoredMarketConfig(existing.JsonData)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Cấu hình chợ hiện có bị hỏng"})
		return
	}
	...
	if err := ac.DB.Save(&existing).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Cập nhật kết nối chợ MCP thất bại"})
		return
	}
...
func (ac *AdminController) DeleteMCPMarket(c *gin.Context) {
	...
	if err := ac.DB.Where("id = ? AND type = ?", id, mcpmarket.MCPMarketConfigType).Delete(&models.Config{}).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Xóa kết nối chợ MCP thất bại"})
		return
	}
...
func (ac *AdminController) ImportMCPMarketService(c *gin.Context) {
	...
	if len(detail.Endpoints) == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Dịch vụ này hiện chưa có địa chỉ SSE/StreamableHTTP khả dụng, vui lòng kích hoạt hoặc triển khai trên chợ nguồn trước rồi thử lại"})
		return
	}

	current, _, err := ac.loadCurrentMCPConfig()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("Đọc cấu hình MCP hiện có thất bại: %v", err)})
		return
	}
	...
	if len(upserts) == 0 {
		if len(skippedURLs) > 0 {
			c.JSON(http.StatusConflict, gin.H{"error": "Tất cả địa chỉ có thể nhập đều xung đột với URL cấu hình thủ công, đã bỏ qua theo nguyên tắc ưu tiên cấu hình thủ công", "skipped_urls": skippedURLs})
			return
		}
		c.JSON(http.StatusBadRequest, gin.H{"error": "Không phân tích được địa chỉ hợp lệ nào để nhập"})
		return
	}
	...
	if err := tx.Commit().Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("Lưu trữ dịch vụ đã nhập thất bại: %v", err)})
		return
	}
...
func (ac *AdminController) loadEnabledMarketsWithAuth() ([]marketWithAuth, error) {
	...
	for _, cfg := range configs {
		...
		if err != nil {
			return nil, fmt.Errorf("Chợ %s giải mã thông tin xác thực thất bại: %w", cfg.Name, err)
		}
...
func (ac *AdminController) getMarketConfigByID(id uint, requireEnabled bool) (models.Config, mcpmarket.MarketConnection, mcpmarket.AuthConfig, error) {
	...
	if requireEnabled && (!cfg.Enabled || !marketCfg.Enabled) {
		return models.Config{}, mcpmarket.MarketConnection{}, mcpmarket.AuthConfig{}, fmt.Errorf("Kết nối chợ đã bị tắt")
	}
	authCfg, err := decodeMarketAuthConfig(marketCfg)
	if err != nil {
		return models.Config{}, mcpmarket.MarketConnection{}, mcpmarket.AuthConfig{}, err
...
func fetchServiceDetail(ctx context.Context, marketModel models.Config, marketCfg mcpmarket.MarketConnection, authCfg mcpmarket.AuthConfig, serviceID string) (*mcpmarket.MarketServiceDetail, error) {
	if mcpmarket.NormalizeProviderID(marketCfg.ProviderID) == mcpmarket.ProviderModelScope && strings.TrimSpace(authCfg.Token) == "" {
		return nil, fmt.Errorf("Chi tiết dịch vụ ModelScope cần Token, vui lòng điền Token trong kết nối chợ này")
	}
	...
	if err != nil {
		if mcpmarket.NormalizeProviderID(marketCfg.ProviderID) == mcpmarket.ProviderModelScope {
			...
			if strings.Contains(errMsg, "invalidauthentication") || strings.Contains(errMsg, "authentication failed") || strings.Contains(errMsg, "401") {
				return nil, fmt.Errorf("Xác thực chi tiết dịch vụ ModelScope thất bại, vui lòng kiểm tra Token trong kết nối chợ có hợp lệ không")
			}
		}
	...
	if mcpmarket.NormalizeProviderID(marketCfg.ProviderID) == mcpmarket.ProviderModelScope && len(detail.Endpoints) == 0 {
		return nil, fmt.Errorf("Dịch vụ này hiện chưa có địa chỉ khả dụng (có thể chưa hoàn tất kích hoạt trên ModelScope), vui lòng kích hoạt trên ModelScope trước rồi thử lại")
	}
	return detail, nil
}
...
func fetchMarketCatalog(ctx context.Context, marketCfg mcpmarket.MarketConnection, authCfg mcpmarket.AuthConfig) (interface{}, error) {
	...
	if mcpmarket.NormalizeProviderID(marketCfg.ProviderID) == mcpmarket.ProviderModelScope {
		if strings.TrimSpace(authCfg.Token) == "" {
			return nil, fmt.Errorf("Chợ ModelScope chỉ lấy các dịch vụ đã kích hoạt, vui lòng điền Token trong kết nối chợ này trước")
		}
...
func mergeEndpointsIntoMCPConfig(current mcpmarket.MergedMCPConfig, detail *mcpmarket.MarketServiceDetail, nameOverride string) (mcpmarket.MergedMCPConfig, []string, string, error) {
	...
	for idx, endpoint := range detail.Endpoints {
		...
		if _, exists := existingURLSet[normURL]; exists {
			return mcpmarket.MergedMCPConfig{}, nil, endpoint.URL, fmt.Errorf("URL xung đột, đã tồn tại địa chỉ dịch vụ MCP giống nhau")
		}
...
func buildStoredMarketConfig(req upsertMCPMarketRequest, existing *mcpmarket.MarketConnection) (mcpmarket.MarketConnection, error) {
	...
	if _, err := url.ParseRequestURI(catalogURL); err != nil {
		return mcpmarket.MarketConnection{}, fmt.Errorf("catalog_url không đúng định dạng")
	}
	if detailTemplate != "" {
		...
		if _, err := url.ParseRequestURI(validateURL); err != nil {
			return mcpmarket.MarketConnection{}, fmt.Errorf("detail_url_template không đúng định dạng")
		}
	}
	...
	if req.Auth.ExtraHeaders != nil {
		...
		if err != nil {
			return mcpmarket.MarketConnection{}, fmt.Errorf("extra_headers sai định dạng")
		}
...
func parseStoredMarketConfig(jsonData string) (mcpmarket.MarketConnection, error) {
	...
	if cfg.Name == "" || cfg.CatalogURL == "" {
		return mcpmarket.MarketConnection{}, fmt.Errorf("Cấu hình chợ thiếu name/catalog_url")
	}
	return cfg, nil
}
...
func decodeMarketAuthConfig(cfg mcpmarket.MarketConnection) (mcpmarket.AuthConfig, error) {
	...
	if cfg.TokenCiphertext != "" {
		...
		// Tương thích lưu trữ mã hóa cũ: nếu đã cấu hình khóa cũ, ưu tiên giải mã và ghi đè
