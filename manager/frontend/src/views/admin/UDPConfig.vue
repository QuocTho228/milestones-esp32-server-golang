  ...
  <div class="udp-config">
    <el-form ref="formRef" :model="form" :rules="rules" class="config-form" v-loading="loading">
            ...
            <div class="card-head">
              <div>
                ...
                <p class="card-description">
                  Cấu hình địa chỉ lắng nghe của UDP Server tích hợp sẵn trong chương trình, để thiết bị có thể phát
                  hiện và kết nối.
                </p>
              </div>
              <el-tag :type="listenReady ? 'success' : 'warning'" effect="plain" round>
                {{ listenReady ? 'Tham số lắng nghe đầy đủ' : 'Chờ bổ sung' }}
              </el-tag>
            </div>
          </template>
          ...
          <template #header>
            <div class="card-head">
              <div>
                ...
                <p class="card-description">
                  Nhập địa chỉ giao thức gửi địa chỉ xuống thiết bị, thiết bị phải thực sự kết nối thành công.
                </p>
              </div>
              <el-tag :type="externalReady ? 'success' : 'warning'" effect="plain" round>
                {{ externalReady ? 'Địa chỉ gửi xuống đầy đủ' : 'Chờ bổ sung' }}
              </el-tag>
            </div>
          </template>
          ...
          <div class="field-stack">
            ...
            <el-form-item label="Cổng bên ngoài" prop="external_port">
              <el-input-number
                ...
                style="width: 100%"
              />
              <div class="field-help">Thiết bị sẽ nhận host và cổng ở đây, không phải Host lắng nghe nội bộ.</div>
            </el-form-item>
          </div>
        </el-card>
      </div>
      ...
      <div class="footer-bar">
        <p class="footer-note">
          Lưu sau khi lưu sẽ cập nhật cấu hình UDP mặc định, dùng cho quá trình phát hiện thiết bị và kết nối tiếp theo.
        </p>
...
const rules = {
  ...
  external_host: [{ required: true, message: 'Vui lòng nhậpHost bên ngoàiđịa chỉ', trigger: 'blur' }],
  external_port: [
    { required: true, message: 'Vui lòng nhậpCổng bên ngoàisố', trigger: 'blur' },
    { type: 'number', min: 1, max: 65535, message: 'Số cổng phải nằm trong khoảng 1-65535', trigger: 'blur' },
  ],
  listen_host: [{ required: true, message: 'Vui lòng nhậpHost lắng ngheđịa chỉ', trigger: 'blur' }],
  listen_port: [
    { required: true, message: 'Vui lòng nhậpCổng lắng nghesố', trigger: 'blur' },
    { type: 'number', min: 1, max: 65535, message: 'Số cổng phải nằm trong khoảng 1-65535', trigger: 'blur' },
  ],
};
  ...
  try {
    ...
    if (configs.length > 0) {
      ...
      try {
        ...
        ElMessage.warning('Định dạng cấu hình UDP bất thường, đã khôi phục về mặc định');
        configData = {};
      }
...
const handleSave = async () => {
  ...
  try {
    const payload = {
      name: form.name,
      config_id: `udp_${String(form.name || '')
