...
export function resolveVADProvider(provider, configId, data = {}) {
  return resolve(
    ...
    (value) => {
      ...
      if (has(value, "model_path", "min_silence_duration_ms"))
        return "silero_vad";
      if (
        has(
          value,
...
export function resolveASRProvider(provider, configId, data = {}) {
  return resolve(
    ...
    (value) => {
      ...
      if (includes(model, "qwen3-asr") || includes(wsUrl, "/realtime"))
        return "aliyun_qwen3";
      if (includes(model, "fun-asr") || includes(wsUrl, "/inference"))
    ...
    (value) => {
      ...
      if (includes(url, "xfyun.cn"))
        return has(
          ...
          : "xunfei";
      if (
        includes(url, "dashscope.aliyuncs.com") ||
        includes(model, "qwen") ||
        has(value, "language_type", "region")
      )
        return "aliyun_qwen";
      if (includes(url, "bigmodel.cn") || includes(model, "glm-tts"))
        return "zhipu";
      if (includes(url, "minimax")) return "minimax";
      if (includes(model, "indextts")) return "indextts_vllm";
      if (includes(url, "openspeech", "volces.com", "volcengine"))
        return has(value, "ws_url", "ws_host", "use_stream", "resource_id")
          ? "doubao_ws"
          : "doubao";
      if (has(value, "api_key", "model", "voice", "response_format"))
        return "openai";
      return "";
    },
    "doubao_ws",
...
export function resolveVisionProvider(provider, configId, data = {}) {
  return resolve(
    ...
    (value) => {
      ...
      if (
        includes(baseUrl, "dashscope.aliyuncs.com") ||
        includes(model, "qwen-vl")
      )
        return "aliyun_vision";
      if (
        includes(baseUrl, "volces.com", "volcengine") ||
        includes(model, "doubao")
      )
