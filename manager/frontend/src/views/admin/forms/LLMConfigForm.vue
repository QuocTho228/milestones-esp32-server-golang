<template>
  <el-form ref="formRef" :model="model" :rules="rules" label-width="150px">
    ...
    <el-form-item v-if="isDify || isCoze" label="Tiền tố User" prop="user_prefix">
      <el-input v-model="model.user_prefix" placeholder="Tùy chọn, mặc định là milestones" />
    </el-form-item>

    <el-form-item v-if="isCoze" label="Connector ID" prop="connector_id">
      <el-input v-model="model.connector_id" placeholder="Tùy chọn, mặc định là 1024" />
    </el-form-item>
    ...
    <el-form-item v-if="isOpenAIOrOllama && requestCapabilityHint" label="Gợi ý tham số">
      <el-alert :title="requestCapabilityHint" type="info" :closable="false" show-icon />
    </el-form-item>
    ...
    <template v-if="thinkingConfig.visible">
      ...
      <el-form-item v-if="thinkingConfig.showEffort" label="Mức độ suy luận" prop="thinking_effort">
        <el-select v-model="model.thinking_effort" placeholder="Vui lòng chọn mức độ suy luận" style="width: 100%">
          <el-option
      ...
      <el-form-item v-if="thinkingConfig.showClearThinking" label="Chuỗi suy luận lịch sử">
        <el-select
          v-model="model.thinking_clear_thinking"
          placeholder="Vui lòng chọn cách xử lý chuỗi suy luận lịch sử"
          style="width: 100%"
        >
          <el-option
      ...
      <el-form-item>
        <el-alert :title="thinkingConfig.hint" type="warning" :closable="false" show-icon />
      </el-form-item>
    </template>
  </el-form>
</template>
...
const requestCapabilityHint = computed(() => {
  ...
  return `Tài liệu không khuyến nghị đặt riêng các tham số sau cho mô hình hiện tại: ${blockedFields.join('、')} — các trường này sẽ không được truyền khi lưu.`;
});
...
const thinkingConfig = computed(() => {
  ...
  return {
    ...
    hint:
      config.hint ||
      'Khi bật chế độ suy luận sâu mạnh hơn, mô hình thường mất nhiều thời gian hơn để suy luận, tốc độ phản hồi đầu tiên và toàn bộ phản hồi sẽ chậm hơn đáng kể.',
  };
});
...
function normalizeThinkingState(provider) {
  ...
  if (
    props.model.thinking_budget_tokens !== null &&
    props.model.thinking_budget_tokens !== undefined &&
    props.model.thinking_budget_tokens !== ''
  ) {
  ...
  if (
    props.model.thinking_clear_thinking === undefined ||
    props.model.thinking_clear_thinking === null ||
    props.model.thinking_clear_thinking === ''
  ) {
    props.model.thinking_clear_thinking = 'default';
  }
}
...
function getJsonData() {
  ...
  if (
    requestConfig.value.allowMaxTokens &&
    m.max_tokens !== undefined &&
    m.max_tokens !== null &&
    m.max_tokens !== ''
  ) {
    config.max_tokens = m.max_tokens;
  }
  if (requestConfig.value.allowTemperature && m.temperature !== undefined && m.temperature !== null)
...
function buildThinkingPayload(model) {
  ...
  const payload = { mode };
  if (
    (config.showBudgetFor || []).includes(mode) &&
    model?.thinking_budget_tokens !== null &&
...
function validate(callback) {
  if (callback) {
    return formRef.value?.validate((valid) => {
      ...
      if (
        finalValid &&
        thinkingConfig.value.showBudget &&
        thinkingConfig.value.budgetRequired &&
        (props.model?.thinking_budget_tokens === null ||
          props.model?.thinking_budget_tokens === undefined ||
          props.model?.thinking_budget_tokens === '')
      ) {
      ...
      if (
        ...
        props.model?.thinking_budget_tokens !== undefined &&
        props.model?.thinking_budget_tokens !== '' &&
        Number(props.model?.thinking_budget_tokens) < thinkingConfig.value.budgetMin
      ) {
        finalValid = false;
      }
      callback(finalValid);
  ...
  return formRef.value?.validate().then(() => {
    ...
    if (
      ...
      return Promise.reject(new Error('Vui lòng điền ngân sách suy luận'));
    }
    if (
      thinkingConfig.value.showBudget &&
      props.model?.thinking_budget_tokens !== null &&
