<template>
  <div class="config-page">
    <el-card v-loading="loading" class="config-card">
      <el-alert title="Thông báo" type="info" :closable="false" show-icon style="margin-bottom: 20px">
        <template #default>
          Nếu triển khai môi trường docker-compose sẽ đọc địa chỉ api từ biến môi trường, không cần cấu hình
        </template>
      </el-alert>
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="180px">
        <el-form-item label="Địa chỉ dịch vụ" prop="base_url">
          <el-input
            ...
            placeholder="Vui lòng nhập địa chỉ dịch vụ HTTP, ví dụ: http://192.168.1.97:8080"
            style="width: 100%"
          />
          <div class="form-tip">
            <el-icon><InfoFilled /></el-icon>
            Vui lòng nhập địa chỉ HTTP, hệ thống sẽ tự động chuyển đổi thành địa chỉ WebSocket
          </div>
        </el-form-item>

        <el-form-item label="Ngưỡng nhận dạng" prop="threshold">
          <el-input-number
          ...
          <div class="form-tip">
            <el-icon><InfoFilled /></el-icon>
            Ngưỡng nhận dạng giọng nói, phạm vi 0.0-1.0, mặc định 0.4. Giá trị càng cao càng nghiêm ngặt
          </div>
        </el-form-item>

        <el-form-item label="Trạng thái kích hoạt">
          <el-switch v-model="form.enabled" />
        </el-form-item>
      </el-form>

      <div class="form-actions">
        <el-button type="primary" @click="handleSave" :loading="saving"> Lưu cấu hình </el-button>
      </div>
    </el-card>
  </div>
</template>
...
const rules = {
  base_url: [
    ...
    {
      pattern: /^https?:\/\/.+/,
      message: 'Vui lòng nhập địa chỉ HTTP hợp lệ, ví dụ: http://192.168.1.97:8080',
      trigger: 'blur',
    },
  ],
  threshold: [
    { required: true, message: 'Vui lòng nhập ngưỡng nhận dạng', trigger: 'blur' },
    {
      type: 'number',
      min: 0,
      max: 1,
      message: 'Ngưỡng phải trong khoảng 0.0 đến 1.0',
      trigger: 'blur',
    },
  ],
};
...
const loadConfig = async () => {
  ...
  try {
    ...
    if (configs.length > 0) {
      // Nếu có cấu hình, sử dụng cấu hình đầu tiên (chỉ nên có một)
      ...
      // Công tắc tương ứng với json_data.enable (kích hoạt dịch vụ), không sử dụng cột enabled trả về từ API
...
const handleSave = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      saving.value = true;
      try {
        // Xây dựng dữ liệu cấu hình: công tắc ghi vào json_data.enable, xuất ra dựa trên trường này
        const configData = {
          service: {
        ...
        const saveData = {
          name: 'Cấu hình nhận dạng giọng nói',
