<template>
  <div class="roles-page">
      ...
      <div v-for="role in roles" :key="role.id" class="role-col">
        <el-card class="role-card" shadow="hover">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    ...
                    <el-dropdown-item command="toggle-status">
                      ...
                      {{ isRoleActive(role) ? 'Tắt' : 'Bật' }}
                    </el-dropdown-item>
                    <el-dropdown-item command="set-default" :disabled="role.is_default">
                      <el-icon><Star /></el-icon>
                      {{ role.is_default ? 'Đã mặc định' : 'Đặt làm mặc định' }}
                    </el-dropdown-item>
          ...
          <div class="role-content">
            <p class="description">{{ role.description || 'Chưa có mô tả' }}</p>

            <div class="role-config">
              <el-tag size="small" :type="isRoleActive(role) ? 'success' : 'info'">
                {{ isRoleActive(role) ? 'Bật' : 'Tắt' }}
              </el-tag>
              <el-tag v-if="role.is_default" size="small" type="warning">Vai trò mặc định</el-tag>
              <el-tag size="small" type="primary">LLM: {{ role.llm_config_id || 'Mặc định' }}</el-tag>
              <el-tag size="small" type="success">TTS: {{ role.tts_config_id || 'Mặc định' }}</el-tag>
              <el-tag v-if="role.voice" size="small" type="warning">Giọng đọc: {{ role.voice }}</el-tag>
            </div>

            <div class="role-prompt">
              <p class="prompt-label">Prompt</p>
              <p class="prompt-text">{{ role.prompt || 'Chưa thiết lập prompt' }}</p>
            </div>
          </div>
        </el-card>
      </div>
    ...
    <el-empty
      ...
      description="Chưa có vai trò toàn cục, nhấn nút phía trên để tạo mới"
    >
      <el-button type="primary" @click="showCreateDialog = true">Tạo vai trò toàn cục đầu tiên</el-button>
    </el-empty>

    <el-dialog
      v-model="showCreateDialog"
      :title="editingRole ? 'Chỉnh sửa vai trò toàn cục' : 'Tạo vai trò toàn cục'"
      width="800px"
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        <div class="dialog-sections">
          <section class="dialog-section">
            <h4 class="dialog-section-title">Thông tin cơ bản</h4>
            <el-form-item label="Tên vai trò" prop="name">
              <el-input v-model="form.name" placeholder="Vui lòng nhập tên vai trò" />
            </el-form-item>
            ...
            <el-form-item label="Mô tả" prop="description">
              <el-input
                ...
                type="textarea"
                :rows="3"
                placeholder="Vui lòng nhập mô tả vai trò"
              />
            </el-form-item>
            ...
            <el-form-item label="Vai trò mặc định">
          ...
          <section class="dialog-section">
            ...
            <el-form-item label="Prompt hệ thống" prop="prompt">
              <el-input
                v-model="form.prompt"
                type="textarea"
                :rows="6"
                placeholder="Vui lòng nhập prompt hệ thống, dùng để định nghĩa hành vi và tính cách của vai trò"
              />
              <div class="prompt-tips">
                <el-text size="small" type="info">
                  Gợi ý: Có thể dùng &#123;&#123;assistant_name&#125;&#125; làm placeholder cho tên agent
                </el-text>
              </div>
            </el-form-item>
          </section>
          ...
          <section class="dialog-section">
            <h4 class="dialog-section-title">Cấu hình mô hình</h4>
            <el-form-item label="Cấu hình LLM">
              <el-select
                v-model="form.llm_config_id"
                placeholder="Chọn cấu hình LLM (tùy chọn)"
                clearable
                style="width: 100%"
              >
                <el-option
                ...
                >
                  ...
                  <el-tag v-if="config.is_default" size="small" type="success" style="margin-left: 8px"
                    >Mặc định</el-tag
                  >
                </el-option>
              </el-select>
              <div class="form-tip">
                <el-text size="small" type="info">Để trống sẽ dùng cấu hình mặc định</el-text>
              </div>
            </el-form-item>
            ...
            <el-form-item label="Cấu hình TTS">
              <el-select
                ...
                placeholder="Chọn cấu hình TTS (tùy chọn)"
                clearable
                style="width: 100%"
            ...
            <el-form-item label="Giọng đọc" v-if="form.tts_config_id">
              <el-select
                v-model="form.voice"
                placeholder="Chọn hoặc nhập giọng đọc (hỗ trợ tìm kiếm và nhập tự do)"
                clearable
                filterable
                allow-create
                ...
                style="width: 100%"
              >
                <el-option v-for="voice in filteredVoices" :key="voice.value" :label="voice.label" :value="voice.value">
              ...
              <div class="form-tip">
                <el-text size="small" type="info"
                  >Tự động tải danh sách giọng đọc theo cấu hình TTS hiện tại, có thể tìm kiếm hoặc nhập thủ
                  công</el-text
                >
              </div>
            </el-form-item>
...
const rules = {
  ...
  prompt: [{ required: true, message: 'Vui lòng nhập prompt hệ thống', trigger: 'blur' }],
};
...
const loadRoles = async () => {
  ...
  try {
    ...
    ElMessage.error('Tải danh sách vai trò thất bại');
  } finally {
    loading.value = false;
  }
};
...
const loadConfigs = async () => {
  try {
    const [llmRes, ttsRes] = await Promise.all([api.get('/admin/llm-configs'), api.get('/admin/tts-configs')]);
    llmConfigs.value = llmRes.data.data || [];
    ttsConfigs.value = ttsRes.data.data || [];
  } catch (error) {
    console.error('Tải danh sách cấu hình thất bại', error);
  }
};
...
const filterVoice = (val) => {
  ...
  filteredVoices.value = availableVoices.value.filter(
    (voice) => voice.label.toLowerCase().includes(keyword) || voice.value.toLowerCase().includes(keyword),
  );
};
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        ElMessage.error('Lưu thất bại: ' + (error.response?.data?.error || error.message));
      } finally {
        saving.value = false;
      }
    }
...
const toggleRoleStatus = async (role) => {
  ...
  const action = isRoleActive(role) ? 'Tắt' : 'Bật';
  try {
    await api.patch(`/admin/roles/global/${role.id}/toggle`);
    ElMessage.success(`${action.charAt(0).toUpperCase() + action.slice(1)} vai trò thành công`);
    await loadRoles();
  } catch (error) {
    ElMessage.error('Thay đổi trạng thái thất bại: ' + (error.response?.data?.error || error.message));
  }
};
...
const setDefaultRole = async (role) => {
  ...
  try {
    ...
    ElMessage.success('Đã đặt làm vai trò mặc định');
    await loadRoles();
  } catch (error) {
    ElMessage.error('Đặt mặc định thất bại: ' + (error.response?.data?.error || error.message));
  }
};

const deleteRole = async (id) => {
  try {
    await ElMessageBox.confirm('Bạn có chắc muốn xóa vai trò toàn cục này không?', 'Thông báo', {
