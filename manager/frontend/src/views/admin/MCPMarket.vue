<template>
  <div class="mcp-market-page">
          ...
          <el-col :xs="24" :lg="11">
            <el-card shadow="never" class="panel-card">
              ...
              <el-table :data="markets" stripe v-loading="marketsLoading" height="560">
                ...
                <el-table-column prop="catalog_url" label="URL danh mục" min-width="220" show-overflow-tooltip />
          ...
          <el-col :xs="24" :lg="13">
            <el-card shadow="never" class="panel-card">
                ...
                <div class="panel-header">
                  ...
                  <div class="search-actions">
                    <el-input
                      ...
                      placeholder="Tìm kiếm tên/mô tả/ID dịch vụ"
                      clearable
                      size="small"
                      style="width: 240px"
              ...
              <el-table :data="services" stripe v-loading="servicesLoading" height="500">
                ...
                <el-table-column prop="market_name" label="Thị trường nguồn" min-width="120" show-overflow-tooltip />
              ...
              <el-alert
                ...
                type="warning"
                :closable="false"
                title="Một số thị trường tải thất bại"
                class="warning-alert"
              >
                <template #default>
      ...
      <el-tab-pane name="imported">
        ...
        <el-card shadow="never" class="panel-card">
            ...
            <div class="panel-header">
              ...
              <div class="search-actions">
                <el-input
                  ...
                  placeholder="Tìm kiếm tên / service_id / URL"
                  clearable
                  size="small"
                  style="width: 320px"
          ...
          <el-table :data="importedItems" stripe v-loading="importedLoading" height="560">
            ...
            <el-table-column prop="transport" label="Giao thức" width="140" />
            ...
            <el-table-column label="Công cụ" width="120">
              <template #default="{ row }">
                <el-tag size="small" :type="row.allowed_tools?.length ? 'warning' : 'info'">
                  {{ row.allowed_tools?.length ? `${row.allowed_tools.length} đã chọn` : 'Tất cả công cụ' }}
                </el-tag>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="enabled" label="Kích hoạt" width="90">
              <template #default="{ row }">
                <el-tag size="small" :type="row.enabled ? 'success' : 'info'">
                  {{ row.enabled ? 'Bật' : 'Tắt' }}
                </el-tag>
            ...
            <el-table-column label="Thao tác" width="280" fixed="right">
              <template #default="{ row }">
                <el-button link type="primary" @click="openEditImportedDialog(row)">Chỉnh sửa</el-button>
    ...
    <el-dialog v-model="detailDialogVisible" title="Chi tiết dịch vụ" width="900px">
      <div v-loading="detailLoading">
        ...
        <template v-else-if="serviceDetail">
          <div class="detail-grid">
            ...
            <div><strong>Thị trường nguồn: </strong>{{ serviceDetail.market_name || '-' }}</div>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="detailImporting" :disabled="!serviceDetail" @click="importFromDetail">
          Nhập cấu hình dịch vụ và cập nhật nóng
        </el-button>
      </template>
    </el-dialog>

    <el-dialog
      v-model="importedDialogVisible"
      :title="editingImported ? 'Chỉnh sửa dịch vụ đã nhập' : 'Thêm dịch vụ mới'"
      width="700px"
    >
      <el-form ref="importedFormRef" :model="importedForm" :rules="importedRules" label-width="120px">
        <el-form-item label="Tên" prop="name">
          <el-input v-model="importedForm.name" placeholder="Tên hiển thị dịch vụ" />
        </el-form-item>
        <el-form-item label="Kích hoạt">
          <el-switch v-model="importedForm.enabled" />
        </el-form-item>
        <el-form-item label="Giao thức" prop="transport">
        ...
        <el-form-item label="Thị trường nguồn">
        ...
        <el-form-item label="Service ID">
          <el-input v-model="importedForm.service_id" placeholder="ID dịch vụ nguồn (tùy chọn)" />
        </el-form-item>
        <el-form-item label="Tên dịch vụ">
          <el-input v-model="importedForm.service_name" placeholder="Tên dịch vụ nguồn (tùy chọn)" />
        </el-form-item>
      ...
      <div class="tool-selector-card">
        <div class="tool-selector-header">
          <div class="tool-selector-meta">
            ...
            <span class="tool-selector-tip">Danh sách trống cho phép tất cả công cụ của dịch vụ này.</span>
          </div>
        ...
        <el-radio-group
          v-model="importedToolMode"
          size="small"
          class="tool-mode-group"
          @change="handleImportedToolModeChange"
        >
        ...
        <template v-if="importedToolMode === 'selected'">
          <div class="tool-picker-search">
            <el-input v-model="importedToolQuery" clearable placeholder="Tìm kiếm tên hoặc mô tả công cụ">
              <template #prefix>
                <el-icon><Search /></el-icon>
              </template>
            </el-input>
          ...
          <div v-if="filteredImportedToolOptions.length === 0" class="tool-picker-empty">
            {{
              importedToolOptions.length === 0
                ? 'Chưa dò tìm được công cụ nào, vui lòng nhấn "Dò tìm công cụ" trước.'
    ...
    <el-dialog
      ...
      :title="editingMarket ? 'Chỉnh sửa thị trường MCP' : 'Thêm thị trường MCP'"
      width="640px"
    >
      <el-form ref="marketFormRef" :model="marketForm" :rules="marketRules" label-width="130px">
        <el-form-item label="Nhà cung cấp">
          <el-select v-model="marketForm.provider_id" style="width: 100%" @change="handleProviderChange">
            <el-option
              v-for="provider in selectableProviderOptions"
              :key="provider.id"
              :label="provider.name"
              :value="provider.id"
            />
        ...
        <el-form-item label="Tên" prop="name">
          <el-input v-model="marketForm.name" placeholder="Ví dụ: Thị trường MCP ModelScope" />
        </el-form-item>
        <el-form-item label="URL danh mục" prop="catalog_url">
          <el-input v-model="marketForm.catalog_url" placeholder="https://example.com/api/services" />
        </el-form-item>
        <el-form-item label="Mẫu URL chi tiết" prop="detail_url_template">
          <el-input
            v-model="marketForm.detail_url_template"
            placeholder="https://example.com/api/services/{id} (tùy chọn)"
          />
        </el-form-item>
        <el-form-item label="Kích hoạt">
        ...
        <el-form-item label="Token">
          <el-input
            ...
            :placeholder="
              editingMarket
                ? `Để trống giữ nguyên giá trị cũ (hiện tại: ${editingMarket.token_mask || 'Chưa thiết lập'})`
                : 'Vui lòng nhập Token ModelScope'
            "
            show-password
            clearable
          />
...
const marketRules = {
  ...
  catalog_url: [{ required: true, message: 'Vui lòng nhập URL danh mục', trigger: 'blur' }],
};
...
const importedRules = {
  name: [{ required: true, message: 'Vui lòng nhập tên', trigger: 'blur' }],
  transport: [{ required: true, message: 'Vui lòng chọn loại giao thức', trigger: 'change' }],
...
const loadProviders = async () => {
  try {
    ...
    ElMessage.error(error.response?.data?.error || 'Tải danh sách nhà cung cấp thất bại');
  }
};
...
const applyProviderPreset = (providerId, force = false) => {
  ...
  if (!editingMarket.value && (force || !marketForm.name) && provider.id === 'modelscope') {
    marketForm.name = 'Thị trường MCP ModelScope';
  }
};
...
const loadMarkets = async () => {
  ...
  try {
    ...
    ElMessage.error(error.response?.data?.error || 'Tải thị trường MCP thất bại');
  } finally {
...
const deleteMarket = async (row) => {
  try {
    await ElMessageBox.confirm(`Xác nhận xóa thị trường MCP「${row.name}」?`, 'Thông báo', {
      type: 'warning',
...
const loadServices = async (page = 1) => {
  ...
  try {
    ...
    ElMessage.error(error.response?.data?.error || 'Tải danh sách dịch vụ tổng hợp thất bại');
  } finally {
...
const importFromDetail = async () => {
  ...
  if (!row?.market_id || !row?.service_id) {
    ElMessage.error('Thiếu định danh dịch vụ, không thể nhập');
    return;
  }
  ...
  try {
    ...
    ElMessage.success(`Nhập thành công: đã áp dụng ${result.imported_count || 0} dịch vụ`);
    await loadServices(servicePage.value);
    await loadImportedItems(1);
    detailDialogVisible.value = false;
    activeTab.value = 'imported';
  } catch (error) {
    ElMessage.error(error.response?.data?.error || 'Nhập thất bại');
  } finally {
...
const loadImportedItems = async (page = 1) => {
  ...
  try {
    ...
    ElMessage.error(error.response?.data?.error || 'Tải danh sách dịch vụ đã nhập thất bại');
  } finally {
...
const parseImportedHeaders = () => {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
      throw new Error('headers phải là đối tượng JSON');
    }
    return parsed;
  } catch (error) {
    throw new Error('Headers không phải là đối tượng JSON hợp lệ');
  }
};
...
const mergeImportedToolOptions = (tools = [], selected = []) => {
  ...
  (selected || []).forEach((name) => {
    ...
    merged.set(name, {
      name,
      description: 'Đang được chọn trong cấu hình hiện tại',
    });
  });
...
const refreshImportedTools = async () => {
  if (!importedToolTarget.value?.id) {
    ElMessage.warning('Vui lòng chọn một dịch vụ đã nhập trước');
    return;
  }
...
const deleteImportedItem = async (row) => {
  try {
    await ElMessageBox.confirm(`Xác nhận xóa dịch vụ đã nhập「${row.name}」?`, 'Thông báo', {
      type: 'warning',
      confirmButtonText: 'Xóa',
      cancelButtonText: 'Hủy',
    });
