  ...
  <div class="agent-config">
    <div class="agent-edit-header">
      <div class="header-left">
        ...
        <h2>{{ form.name || 'Chỉnh sửa trợ lý AI' }}</h2>
      </div>
    ...
    <div class="role-strip" v-loading="rolesLoading">
      ...
      >
        <span>{{ role.name }}</span>
        <small>{{ role.role_type === 'global' ? 'Toàn cục' : 'Của tôi' }}</small>
      </button>
      <span v-if="!rolesLoading && allRoles.length === 0" class="role-empty">Chưa có vai trò khả dụng</span>
    </div>
...
const loadAgent = async () => {
  ...
  try {
    ...
    ElMessage.error(error.response?.data?.error || 'Tải cấu hình trợ lý AI thất bại');
  } finally {
...
const applyRoleConfig = async (role) => {
  ...
  try {
    ...
    ElMessage.info('Đã điền cấu hình vai trò, vui lòng nhấn "Lưu cấu hình" để lưu lại');
  } finally {
...
const handleSave = async () => {
  if (applyingRoleConfig.value) {
    ElMessage.info('Đang điền cấu hình vai trò, vui lòng lưu sau');
    return;
  }
