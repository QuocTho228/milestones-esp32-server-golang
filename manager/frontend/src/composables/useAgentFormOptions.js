...
export const DEFAULT_AGENT_PROMPT =
  'Tôi là {{assistant_name}}, một trợ lý học tập thân thiện, luôn sẵn sàng đồng hành cùng các bạn học sinh.\nTôi nói chuyện gần gũi, dễ hiểu, giải thích kiến thức một cách ngắn gọn, rõ ràng và kiên nhẫn.\nTôi thích khuyến khích, động viên mọi người học tập, giúp giải đáp thắc mắc và truyền cảm hứng để việc học trở nên thú vị hơn.';

export const OPENCLAW_DEFAULT_ENTER_KEYWORDS = ['Mở OpenClaw', 'Khởi động OpenClaw'];
export const OPENCLAW_DEFAULT_EXIT_KEYWORDS = ['Đóng OpenClaw', 'Thoát OpenClaw'];
...
export const agentToForm = (agent = {}, { isAdmin = false } = {}) => {
  ...
  return {
    ...
    knowledge_base_ids: Array.isArray(agent.knowledge_base_ids)
      ? agent.knowledge_base_ids.map(Number).filter(Boolean)
      : [],
...
export const buildAgentPayload = (form = {}, { isAdmin = false } = {}) => {
  ...
  const payload = {
    ...
    knowledge_base_ids: Array.isArray(form.knowledge_base_ids)
      ? form.knowledge_base_ids.map(Number).filter(Boolean)
      : [],
...
export const createDefaultDeviceForm = ({
  isAdmin = false,
  userId = null,
  mode = 'create',
  fixedAgentId = null,
...
export const buildDevicePayload = (form = {}, { isAdmin = false, mode = 'create' } = {}) => {
  if (mode === 'bind') {
    ...
    const payload = /^\d{6}$/.test(identifier) ? { code: identifier } : { device_mac: identifier };
...
export function useAgentFormOptions(options = {}) {
  ...
  const loadAgents = async () => {
    ...
    try {
      ...
      agents.value =
        isAdmin.value && targetUserId.value
  ...
  const loadCloneVoices = async (ttsConfigId = '') => {
    ...
    try {
      ...
      const url =
        isAdmin.value && targetUserId.value ? `/admin/users/${targetUserId.value}/voice-clones` : '/user/voice-clones';
      const response = await api.get(url, { params });
