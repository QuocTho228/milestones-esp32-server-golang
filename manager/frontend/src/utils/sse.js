...
const readSSE = async (response, onEvent) => {
  ...
  if (!reader) {
    throw new Error('Trình duyệt không hỗ trợ đọc dữ liệu dạng luồng (streaming)');
  }
...
export const postJSONWithSSE = async ({ url, body, timeoutMs = 0, token = '', onEvent }) => {
  if (!url) {
    throw new Error('Địa chỉ yêu cầu không được để trống');
  }
  ...
  try {
    ...
    if (error && error.name === 'AbortError') {
      throw new Error('Yêu cầu quá thời gian chờ (timeout)');
    }
    throw error;
  } finally {
    if (timer) {
