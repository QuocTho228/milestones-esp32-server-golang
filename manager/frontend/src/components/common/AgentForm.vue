<template>
  ...
  >
    ...
    <div class="agent-form-grid">
      <el-form-item label="Tên tác nhân AI" prop="name">
        <el-input
          v-model="form.name"
          placeholder="Nhập tên tác nhân AI hiển thị ở trang quản trị"
          maxlength="50"
          show-word-limit
        />
      </el-form-item>
      <el-form-item label="Biệt danh tác nhân AI" prop="nickname">
        <el-input
          v-model="form.nickname"
          placeholder="Dùng cho mô hình ngôn ngữ, ví dụ: Minh"
          maxlength="50"
          show-word-limit
        />
      </el-form-item>
    ...
    <el-form-item label="Giới thiệu vai trò" prop="custom_prompt">
      <el-input
        v-model="form.custom_prompt"
        type="textarea"
        :rows="4"
        placeholder="Nhập giới thiệu vai trò / System prompt"
        maxlength="10000"
        show-word-limit
      />
    </el-form-item>
    ...
    <div class="agent-form-grid">
      <el-form-item label="Mô hình ngôn ngữ" prop="llm_config_id">
        <el-select
          v-model="form.llm_config_id"
          placeholder="Chọn mô hình ngôn ngữ"
          style="width: 100%"
          clearable
          filterable
        ...
        >
          <el-option
            ...
            :label="config.is_default ? `${config.name} (Mặc định)` : config.name"
    ...
    <el-form-item v-if="form.tts_config_id" label="Giọng TTS" prop="voice">
      <el-select
        v-model="form.voice"
        placeholder="Chọn hoặc nhập giá trị giọng chính xác"
        style="width: 100%"
        filterable
        allow-create
    ...
    <div class="agent-form-grid agent-form-grid-three">
      <el-form-item label="Tốc độ nhận dạng giọng nói" prop="asr_speed">
        <el-select v-model="form.asr_speed" style="width: 100%">
          <el-option label="Bình thường" value="normal" />
          <el-option label="Kiên nhẫn" value="patient" />
          <el-option label="Nhanh" value="fast" />
        </el-select>
      </el-form-item>
      <el-form-item label="Chế độ bộ nhớ" prop="memory_mode">
        <el-select v-model="form.memory_mode" style="width: 100%">
          <el-option label="Không có bộ nhớ" value="none" />
          <el-option label="Bộ nhớ ngắn" value="short" />
          <el-option label="Bộ nhớ dài" value="long" />
        </el-select>
      </el-form-item>
      <el-form-item label="Giới hạn chat theo giọng nói" prop="speaker_chat_mode">
        <el-select v-model="form.speaker_chat_mode" style="width: 100%">
          <el-option label="Tắt" value="off" />
          <el-option label="Chỉ cho phép chat khi nhận diện đúng giọng" value="identified_only" />
        </el-select>
      </el-form-item>
    </div>

    <el-form-item label="Dịch vụ MCP">
      <el-select
        ...
        clearable
        style="width: 100%"
        placeholder="Để trống sẽ dùng tất cả dịch vụ đang bật"
    ...
    <div class="openclaw-panel">
      <div class="openclaw-switch-row">
        <span>Cho phép vào chế độ OpenClaw</span>
        <el-switch v-model="form.openclaw_allowed" />
      </div>
      <div class="agent-form-grid">
        <el-form-item label="Từ khóa vào OpenClaw">
          <el-select
            ...
            clearable
            style="width: 100%"
            placeholder="Nhập xong nhấn Enter, có thể thêm nhiều từ khóa"
          />
        </el-form-item>
        <el-form-item label="Từ khóa thoát OpenClaw">
          <el-select
...
const rules = computed(() => ({
  ...
  name: [
    { required: true, message: 'Vui lòng nhập tên tác nhân AI', trigger: 'blur' },
    { min: 1, max: 50, message: 'Độ dài không được vượt quá 50 ký tự', trigger: 'blur' },
  ],
  nickname: [
    { required: true, message: 'Vui lòng nhập biệt danh tác nhân AI', trigger: 'blur' },
    { min: 1, max: 50, message: 'Độ dài không được vượt quá 50 ký tự', trigger: 'blur' },
  ],
  asr_speed: [{ required: true, message: 'Vui lòng chọn tốc độ nhận dạng giọng nói', trigger: 'change' }],
  memory_mode: [{ required: true, message: 'Vui lòng chọn chế độ bộ nhớ', trigger: 'change' }],
  speaker_chat_mode: [{ required: true, message: 'Vui lòng chọn giới hạn chat theo giọng nói', trigger: 'change' }],
}));
...
const selectedMcpServices = computed({
  get: () =>
    String(form.value.mcp_service_names || '')
      .split(',')
      .map((item) => item.trim())
      .filter(Boolean),
  set: (items) => {
...
const loadTargetUserOptions = async () => {
  ...
  if (validKnowledgeBaseIds.size) {
    form.value.knowledge_base_ids = (form.value.knowledge_base_ids || []).filter((id) =>
      validKnowledgeBaseIds.has(Number(id)),
    );
