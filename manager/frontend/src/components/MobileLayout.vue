<template>
  <div class="mobile-layout">
    <MobileNavBar :title="pageTitle" :show-back="showBack" class="mobile-nav-bar">
      <template #right>
    ...
    <!-- Lớp menu người dùng hiện lên -->
    <van-popup v-model:show="showUserMenu" position="bottom" :style="{ padding: '20px' }">
      <div class="user-menu">
        <div class="user-info">
        ...
        <van-cell-group inset>
          <van-cell title="Chức năng khác" is-link @click="handleGoMore" />
          <van-cell v-if="!authStore.isAdmin" title="API Token" is-link @click="handleGoApiTokens" />
          <van-cell v-if="authStore.isAdmin" title="Trình hướng dẫn cấu hình" is-link @click="handleGoConfigWizard" />
...
// Có hiển thị thanh tab dưới không
const showTabBar = computed(() => {
  const hideTabBarPages = ['/login', '/setup', '/test', '/simple-login'];
  const currentPath = route.path;

  // Trang chi tiết không hiển thị thanh tab
...
const roleText = computed(() => {
  return authStore.isAdmin ? 'Quản trị viên' : 'Người dùng';
});
...
// Lắng nghe thay đổi tuyến đường, đóng menu người dùng
watch(
  () => route.path,
  () => {
    showUserMenu.value = false;
