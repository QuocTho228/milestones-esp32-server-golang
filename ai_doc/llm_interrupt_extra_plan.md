...
- 中断触发：`/Users/shijingbo/git/milestones-esp32-server-golang/internal/app/server/chat/common.go:3`  
  `StopSpeaking()` 会取消 `SessionCtx`，导致 LLM/TTS 处理上下文结束。

- LLM 流式处理与落历史：`/Users/shijingbo/git/milestones-esp32-server-golang/internal/app/server/chat/llm.go:323`  
  当前 `handleLLMResponse()` 只有在 `llmResponse.IsEnd=true` 时才保存 assistant 消息；  
  `ctx.Done()` 分支直接 return，不会保存“已输出但被打断”的 assistant。

- 历史组装入口：`/Users/shijingbo/git/milestones-esp32-server-golang/internal/app/server/chat/llm.go:1050`  
...
改动位置：`/Users/shijingbo/git/milestones-esp32-server-golang/internal/app/server/chat/llm.go:324` 的 `handleLLMResponse()`
...
改动位置：`/Users/shijingbo/git/milestones-esp32-server-golang/internal/app/server/chat/llm.go:1050` 的 `GetMessages()`
...
- （可选）`/Users/shijingbo/git/milestones-esp32-server-golang/test/interrupt_history/main.go` 用于验证演示
