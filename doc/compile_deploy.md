...
```mermaid
flowchart LR
    Device["ESP32 设备 / WebSocket-MQTT-UDP 客户端"] --> Main["主程序 milestones_server<br/>cmd/server"]
...
3. 如果设备走 WebSocket，核心接入地址通常为 `ws://<host>:8989/milestones/v1/`。
...
因此，最终产物里的 `milestones_server` 实际上是“主程序 + 控制台后端 + 声纹服务 + 已嵌入的控制台前端”。
...
- OTA 接口：`http://<host>:8989/milestones/ota/`
- MQTT：`<host>:2883`
- UDP：`<host>:8990`
...
- 分离部署：四个服务分别构建、分别运行
- AIO 打包：前端、后端、声纹服务被一起编进 `milestones_server`
